...
shared_ptr<PhysicsController> PhysicsFactory::CreateTripod(glm::vec3 position)
{
	// Create constraints
	...
	// Create rigid bodies
	// Body 
	shared_ptr<PhysicsController> bodypart_body = CreateSphere(8.5f, glm::vec3(position.x, position.y + 52, position.z + 2), glm::quat());
	
	// Left Leg
	shared_ptr<PhysicsController> bodypart_left_upper_leg = CreateCylinder(1.4, 11, 
		glm::vec3(position.x - 15, position.y + 62, position.z + 1), 
		glm::angleAxis(67.5f, glm::vec3(0, 0, 1)));

	shared_ptr<PhysicsController> bodypart_left_lower_leg = CreateCylinder(1.4, 50, 
		glm::vec3(position.x - 20, position.y + 33, position.z + 1),
		glm::quat());
	
	shared_ptr<PhysicsController> bodypart_left_foot = CreateBox(5, 5, 5,
		glm::vec3(position.x - 20, position.y + 5, position.z + 1),
		glm::quat());

	shared_ptr <PhysicsController> bodypart_left_shoulder = CreateSphere(1.7f, glm::vec3(position.x - 8, position.y + 59, position.z + 1),
		glm::quat());

	shared_ptr <PhysicsController> bodypart_left_knee = CreateSphere(1.5f, glm::vec3(position.x - 20, position.y + 60, position.z + 1),
		glm::quat());

	// Right Leg
	shared_ptr<PhysicsController> bodypart_right_upper_leg = CreateCylinder(1.4, 11, 
		glm::vec3(position.x + 15, position.y + 62, position.z + 1),
		glm::angleAxis(67.5f, glm::vec3(0, 0, -1)));

	shared_ptr<PhysicsController> bodypart_right_lower_leg = CreateCylinder(1.4, 50, 
		glm::vec3(position.x + 20, position.y + 33, position.z + 1), 
		glm::quat());

	shared_ptr<PhysicsController> bodypart_right_foot = CreateBox(5, 5, 5,
		glm::vec3(position.x + 20, position.y + 5, position.z + 1),
		glm::quat());

	shared_ptr<PhysicsController> bodypart_right_shoulder = CreateSphere(1.7f, glm::vec3(position.x + 8, position.y + 59, position.z + 1),
		glm::quat());

	shared_ptr<PhysicsController> bodypart_right_knee = CreateSphere(1.5f, glm::vec3(position.x + 20, position.y + 60, position.z + 1),
		glm::quat());

	// Back leg
	shared_ptr<PhysicsController> bodypart_back_lower_leg = CreateCylinder(1.4, 50, 
		glm::vec3(position.x, position.y + 33, position.z - 19),
		glm::quat());

	shared_ptr<PhysicsController> bodypart_back_upper_leg = CreateCylinder(1.4, 11, 
		glm::vec3(position.x, position.y + 62, position.z - 13),
		glm::angleAxis(67.5f, glm::vec3(-1, 0, 0)));
	
	shared_ptr<PhysicsController> bodypart_back_foot = CreateBox(5, 5, 5,
		glm::vec3(position.x, position.y + 5, position.z - 19),
		glm::quat());
	
	shared_ptr<PhysicsController> bodypart_back_shoulder = CreateSphere(1.7f, glm::vec3(position.x, position.y + 59, position.z - 6),
		glm::quat());

	shared_ptr<PhysicsController> bodypart_back_knee = CreateSphere(1.5f, glm::vec3(position.x, position.y + 60, position.z - 19),
		glm::quat());
	...
	// Enable motors
	shoulder_right_upper_leg->enableMotor(true);
	shoulder_left_upper_leg->enableMotor(true);
	shoulder_back_upper_leg->enableAngularMotor(true, 0.1, 0.1);
